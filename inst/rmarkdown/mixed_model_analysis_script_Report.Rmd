...
We preprocess the _peptide intensities reported in the MaxQuant generated`peptides.txt` file_ as follows: we removed all proteins with only one identified peptide, we remove intensities equal zero, non zero intensities are $\log_2$ transformed and modified using robust z-score transformation (using the median and median average deviation). 
...
For each protein, we fitted a `r if(pars$model$is_mixed) {mixed_txt} else {fixed_txt}` to the peptide intensities. Fold changes and p-Values for each contrast ( differences among conditions ,see table \@ref(tab:contrasts) ) are computed based on the parameter estimates of the model model.  `r if(pars$model$is_mixed) {mixed_contrast_txt}`. All the protein p-Values are adjusted for multiple testing using the Benjamini and Hochberg procedure to obtain the false discovery rate (FDR) [see @BenjaminiHochberg1995].
...
```{r contrasts}
Contrasts <- data.frame(contrast = names(pars$Contrasts),
                        explicit = as.character(pars$Contrasts))
