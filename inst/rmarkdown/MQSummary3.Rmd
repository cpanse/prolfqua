...
if (!local_config$table$is_intensity_transformed) {
  data_tf <- LFQService::transform_work_intensity(data_cc,
                                                  local_config,
                                                  log2)
  dataTransformed <-
    LFQService::applyToIntensityMatrix(data_tf, local_config, robust_scale)
...
samples <- dplyr::select(dataTransformed, local_config$table$sampleName) %>%
  distinct() %>%
  pull()
...
if (length(samples) > 10) {
  ...
  LFQService::pairs_smooth( toWideConfig(ldata, local_config, as.matrix = TRUE)$data )
}else{
  LFQService::pairs_smooth( toWideConfig(dataTransformed, local_config, as.matrix = TRUE)$data )
}
```
...
LFQService::table_facade(sd_quantile_res2,
